...
const historyList = ref<VideoRecord[]>([])
...
const loadHistory = () => {
  try {
    const data = localStorage.getItem('video_generation_history')
    if (data) {
      const records = JSON.parse(data)
      historyList.value = records.map((record: any) => ({
        id: record.id || Date.now().toString(),
        videoUrl: record.videoUrl || '',
        prompt: record.prompt || '无描述',
        hasImage: record.hasImage || false,
      ...
      console.log('✅ 加载了', historyList.value.length, '条历史记录')
    }
  } catch (error) {
...
const saveHistory = () => {
  try {
    localStorage.setItem('video_generation_history', JSON.stringify(historyList.value))
  } catch (error) {
...
// 下载视频
const downloadVideo = async (record: VideoRecord) => {
  if (!record.videoUrl) {
    ElMessage.error('视频URL不存在')
    return
  }
...
// 查看视频
const viewVideo = (record: VideoRecord) => {
  ...
  <div class="history-page">
    ...
    <div class="container">
      ...
      <div v-if="filteredList.length === 0" class="empty-state">
        ...
        <p>{{ searchKeyword ? '尝试使用其他关键词搜索' : '开始创建您的第一个宣传视频吧' }}</p>
        <button class="start-btn" @click="router.push('/video-generation')">
          <span>+</span> 开始制作
        </button>
      </div>
      ...
      <div v-else class="videos-grid">
        ...
        >
          ...
          <div class="video-thumbnail">
            ...
            <div class="thumbnail-overlay">
              ...
              <button 
                v-if="record.status === 'completed'"
                class="overlay-btn" 
                @click.stop="downloadVideo(record)"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="video-info">
            <h3 class="video-title">{{ record.prompt || '无描述' }}</h3>
            <div class="video-date">
              ...
              <span v-if="record.aspectRatio" class="meta-tag">{{ record.aspectRatio }}</span>
              <span v-if="record.duration" class="meta-tag">{{ record.duration }}秒</span>
            </div>
          </div>
        </div>
      </div>
